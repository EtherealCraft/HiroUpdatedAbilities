...
public class Boid {
	...
	public Boid(Player player, FireSpray fireSpray, Location loc, Vector dir, double damage, double blueFireDamage, double range) {
		this.player = player;
		...
		this.range = range;
		this.damage = damage;
		this.blueFireDamage = blueFireDamage;
	}
	...
	public void show() {
		...
		fireSpray.playFirebendingParticles(this.location, 1, 0.075f, 0.075f, 0.075f);
		...
		for (Entity e : GeneralMethods.getEntitiesAroundPoint(this.location, 1.5)) {
			if (e instanceof LivingEntity && !e.getUniqueId().equals(player.getUniqueId())) {
				if (!BendingPlayer.getBendingPlayer(player).hasElement(Element.BLUE_FIRE))
					DamageHandler.damageEntity(e, this.damage, fireSpray);
				else
					DamageHandler.damageEntity(e, this.blueFireDamage, fireSpray);
